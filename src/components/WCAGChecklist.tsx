    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 mb-4">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-technical font-black uppercase tracking-[0.3em] text-text-muted opacity-60">Compliance Matrix</h3>
          <div className="flex gap-6 text-xs font-technical font-black uppercase tracking-[0.2em] opacity-60">
        ...
        <div className="flex gap-3">
          {(['all', 'fail', 'pass'] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-5 py-2.5 rounded-xl text-xs font-technical font-black uppercase tracking-[0.2em] transition-all border ${
                filter === f 
            ...
            >
              ...
              <div className="flex-1 space-y-3">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-technical font-black text-text-muted opacity-50 group-hover:opacity-80 transition-opacity tracking-[0.1em] uppercase">{issue.criterion}</span>
                  <span className="text-xs font-technical font-black px-3 py-1 rounded-full border border-white/5 bg-white/[0.02] text-text-muted uppercase tracking-[0.1em] opacity-60">
                    Lvl {issue.level}
                  </span>
                </div>
                ...
                <p className="text-sm text-text-secondary font-body font-medium leading-relaxed opacity-80 group-hover:opacity-100 transition-opacity">{issue.description}</p>
                
                {issue.element && (
                  <div className="pt-4">
                    <span className="text-xs uppercase font-technical font-black text-text-muted tracking-[0.3em] opacity-50">Segment Context</span>
                    <div className="mt-3 px-4 py-3 bg-white/[0.02] rounded-2xl border border-white/5 text-xs font-mono text-text-muted/80 break-all leading-relaxed shadow-inner">
                      {issue.element}
                    </div>
                  </div>
                )}
